...
public class Gauge extends Control {
	public enum NeedleType {
		BIG, FAT, STANDARD, SCIENTIFIC, AVIONIC, VARIOMETER
	}
	...
	public enum NeedleSize {
		THIN(0.015), STANDARD(0.025), THICK(0.05);
	...
	public enum ScaleDirection {
		CLOCKWISE, COUNTER_CLOCKWISE, LEFT_TO_RIGHT, RIGHT_TO_LEFT, BOTTOM_TO_TOP, TOP_TO_BOTTOM
	}

	public enum SkinType {
		AMP, BULLET_CHART, DASHBOARD, FLAT, GAUGE, INDICATOR, KPI, MODERN, SIMPLE, SLIM, SPACE_X, QUARTER, HORIZONTAL, VERTICAL, LCD, TINY, BATTERY, LEVEL, LINEAR, DIGITAL, SIMPLE_DIGITAL, SECTION, BAR, WHITE, CHARGE, SIMPLE_SECTION, TILE_KPI, TILE_TEXT_KPI, TILE_SPARK_LINE
	}
	...
	private static ScheduledExecutorService blinkService = new ScheduledThreadPoolExecutor(1,
			Helper.getThreadFactory("BlinkTask", true));
	...
	private void init() {
		...
		value = new DoublePropertyBase(_minValue) {
			...
			protected void invalidated() {
				...
				if (isAnimated() && withinSpeedLimit) {
					long animationDuration = isReturnToZero() ? (long) (0.2 * getAnimationDuration())
				...
				if (isAveragingEnabled()) {
					movingAverage.addData(new Data(VALUE));
				}
			}
		...
		timeline.setOnFinished(e -> {
			if (isReturnToZero() && Double.compare(currentValue.get(), 0.0) != 0.0) {
				...
				final KeyFrame KEY_FRAME2 = new KeyFrame(Duration.millis((long) (0.8 * getAnimationDuration())),
						KEY_VALUE2);
	...
	/**
	 * Returns the last value of the Gauge. This will not be the last value
	 * during an animation but the final last value after the animation was
	 * finished. If you need to get the last value during an animation you
	 * should use formerValue instead.
	 *
	 ...
	 * an animation. If you need to get the last value after the animation is
	 * finished or if you don't use animation at all (when using real values)
	 * you should use oldValue instead.
	 *
	...
	public void setMinValue(final double VALUE) {
		...
		if (null == minValue) {
			...
			if (Double.compare(originalMinValue, -Double.MAX_VALUE) == 0)
				originalMinValue = _minValue;
			if (isStartFromZero() && _minValue < 0)
				setValue(0);
			if (Double.compare(originalThreshold, getThreshold()) < 0) {
				setThreshold(Helper.clamp(_minValue, getMaxValue(), originalThreshold));
			}
		...
		if (null == minValue) {
			minValue = new DoublePropertyBase(_minValue) {
				...
				protected void invalidated() {
					...
					if (isStartFromZero() && _minValue < 0)
						Gauge.this.setValue(0);
					if (Double.compare(originalThreshold, getThreshold()) < 0) {
						setThreshold(Helper.clamp(VALUE, getMaxValue(), originalThreshold));
					}
					fireUpdateEvent(RECALC_EVENT);
					if (!valueProperty().isBound())
						Gauge.this.setValue(Helper.clamp(getMinValue(), getMaxValue(), Gauge.this.getValue()));
	...
	/**
	 * Returns the maximum value of the scale. This value represents the upper
	 * limit of the visible gauge values.
	 *
	...
	public void setMaxValue(final double VALUE) {
		...
		if (null == maxValue) {
			...
			if (Double.compare(originalMaxValue, Double.MAX_VALUE) == 0)
				originalMaxValue = _maxValue;
			if (Double.compare(originalThreshold, getThreshold()) > 0) {
				setThreshold(Helper.clamp(getMinValue(), _maxValue, originalThreshold));
			}
			if (!valueProperty().isBound())
				Gauge.this.setValue(Helper.clamp(getMinValue(), getMaxValue(), Gauge.this.getValue()));
		} else {
		...
		if (null == maxValue) {
			maxValue = new DoublePropertyBase(_maxValue) {
				...
				protected void invalidated() {
					...
					if (Double.compare(originalThreshold, getThreshold()) > 0) {
						setThreshold(Helper.clamp(getMinValue(), VALUE, originalThreshold));
					}
					fireUpdateEvent(RECALC_EVENT);
					if (!valueProperty().isBound())
						Gauge.this.setValue(Helper.clamp(getMinValue(), getMaxValue(), Gauge.this.getValue()));
	...
	/**
	 * Always returns the range of the gauge scale (maxValue - minValue). This
	 * value will be automatically calculated each time the min- or maxValue
	 * will change.
	 *
	 * @return the range of the gauge scale
	 */
	public double getRange() {
		return null == range ? _range : range.get();
	}

	/**
	 * This is a private method that sets the range to the given value which is
	 * always (maxValue - minValue).
	 *
	 * @param RANGE
	 */
	...
	public ReadOnlyDoubleProperty rangeProperty() {
		if (null == range) {
			range = new DoublePropertyBase((getMaxValue() - getMinValue())) {
				@Override
				protected void invalidated() {
					setAngleStep(getAngleRange() / get());
				}
	...
	/**
	 * Returns the threshold value that can be used to visualize a threshold
	 * value on the scale. There are also events that will be fired if the
	 * threshold was exceeded or underrun. The value will be clamped to range of
	 * the gauge.
	 *
	...
	/**
	 * Sets the threshold of the gauge to the given value. The value will be
	 * clamped to the range of the gauge.
	 *
	 * @param THRESHOLD
	 */
			...
			threshold = new DoublePropertyBase(_threshold) {
				...
				protected void invalidated() {
					...
					if (THRESHOLD < getMinValue() || THRESHOLD > getMaxValue())
						set(Helper.clamp(getMinValue(), getMaxValue(), THRESHOLD));
					fireUpdateEvent(RESIZE_EVENT);
				}
	...
	/**
	 * Returns the title of the gauge. This title will usually only be visible
	 * if it is not empty.
	 *
	...
	/**
	 * Sets the title of the gauge. This title will only be visible if it is not
	 * empty.
	 *
	 * @param TITLE
	 */
	...
	/**
	 * Returns the subtitle of the gauge. This subtitle will usually only be
	 * visible if it is not empty.
	 *
	 * @return the subtitle of the gauge
	 */
	public String getSubTitle() {
		return null == subTitle ? _subTitle : subTitle.get();
	}

	/**
	 * Sets the subtitle of the gauge. This subtitle will usually only be
	 * visible if it is not empty.
	 *
	 * @param SUBTITLE
	 */
	...
	/**
	 * Returns the unit of the gauge. This unit will usually only be visible if
	 * it is not empty.
	 *
	...
	/**
	 * Sets the unit of the gauge. This unit will usually only be visible if it
	 * is not empty.
	 *
	 * @param UNIT
	 */
	...
	public boolean isAveragingEnabled() {
		return null == averagingEnabled ? _averagingEnabled : averagingEnabled.get();
	}
	...
	/**
	 * Returns the number of values that should be used for the averaging of
	 * values. The value must be in the range of 1 - 1000.
	 * 
	 * @return the number of values used for averaging
	 */
	public int getAveragingPeriod() {
		return null == averagingPeriod ? _averagingPeriod : averagingPeriod.get();
	}

	/**
	 * Defines the number values that should be used for the averaging of
	 * values. The value must be in the range of 1 - 1000.
	 * 
	 * @param PERIOD
	 */
	...
	/**
	 * Returns the current list of Data objects that will be used to calculate
	 * the moving average.
	 * 
	 * @return the current list of Data objects used for the moving average
	 */
	public Queue<Data> getAveragingWindow() {
		return movingAverage.getWindow();
	}

	/**
	 * Returns the moving average over the number of values defined by
	 * averagingPeriod.
	 * 
	 * @return the moving the average over the number of values defined by
	 *         averagingPeriod
	 */
	 ...
	 */
	public double getTimeBasedAverageOf(final java.time.Duration DURATION) {
		return movingAverage.getTimeBasedAverageOf(DURATION);
	}

	/**
	 * Returns an observable list of Section objects. The sections will be used
	 * to colorize areas with a special meaning such as the red area in a rpm
	 * gauge. Sections in the Medusa library usually are less eye-catching than
	 * Areas.
	 *
	...
	/**
	 * Sets the sections to the given list of Section objects. The sections will
	 * be used to colorize areas with a special meaning such as the red area in
	 * a rpm gauge. Sections in the Medusa library usually are less eye-catching
	 * than Areas.
	 *
	 * @param SECTIONS
	 */
	...
	/**
	 * Sets the sections to the given array of Section objects. The sections
	 * will be used to colorize areas with a special meaning such as the red
	 * area in a rpm gauge.
	 *
	 * @param SECTIONS
	 */
	...
	/**
	 * Adds the given Section to the list of sections. Sections in the Medusa
	 * library usually are less eye-catching than Areas.
	 *
	 * @param SECTION
	 */
	...
	/**
	 * Removes the given Section from the list of sections. Sections in the
	 * Medusa library usually are less eye-catching than Areas.
	 *
	 * @param SECTION
	 */
	 ...
	 * Returns an observable list of Section objects. The sections will be used
	 * to colorize areas with a special meaning such as the red area in a rpm
	 * gauge. Areas in the Medusa library usually are more eye-catching than
	 * Sections.
	 *
	 * @return an observable list of Section objects
	 */
	 ...
	 * Sets the sections to the given list of Section objects. The sections will
	 * be used to colorize areas with a special meaning such as the red area in
	 * a rpm gauge. Areas in the Medusa library usually are more eye-catching
	 * than Sections.
	 *
	 * @param AREAS
	 */
	 ...
	 * Sets the sections to the given array of Section objects. The sections
	 * will be used to colorize areas with a special meaning such as the red
	 * area in a rpm gauge. Areas in the Medusa library usually are more
	 * eye-catching than Sections.
	 *
	 * @param AREAS
	 */
	public void setAreas(final Section... AREAS) {
		setAreas(Arrays.asList(AREAS));
	}

	/**
	 * Adds the given Section to the list of areas. Areas in the Medusa library
	...
	/**
	 * Removes the given Section from the list of areas. Areas in the Medusa
	 * library usually are more eye-catching than Sections.
	 *
	 * @param AREA
	 */
	...
	/**
	 * Returns an observable list of Section objects. The sections will be used
	 * to colorize tickmarks with a special meaning such as the red area in a
	 * rpm gauge.
	 *
	 * @return an observable list of Section objects
	 */
	 ...
	 * @param SECTIONS
	 */
	public void setTickMarkSections(final Section... SECTIONS) {
		setTickMarkSections(Arrays.asList(SECTIONS));
	}
	...
	/**
	 * Returns an observable list of Section objects. The sections will be used
	 * to colorize ticklabels with a special meaning such as the red area in a
	 * rpm gauge.
	 *
	 * @return an observable list of Section objects
	 */
	public ObservableList<Section> getTickLabelSections() {
		return tickLabelSections;
	}
	 ...
	 * @param SECTIONS
	 */
	public void setTickLabelSections(final Section... SECTIONS) {
		setTickLabelSections(Arrays.asList(SECTIONS));
	}
	...
	/**
	 * Returns an observable list of Marker objects. Like the threshold the
	 * markers are used to visualize specific values. The markers will be
	 * visualized using nodes with mouse event support (pressed, released) and
	 * tooltip.
	 *
	...
	/**
	 * Sets the list of markers to the given list of Marker objects. The markers
	 * will be visualized using nodes with mouse event support (pressed,
	 * released) and tooltip.
	 *
	 * @param MARKERS
	 */
	...
	/**
	 * Sets the list of markers to the given array of Marker objects. The
	 * markers will be visualuzed using nodes with mouse event support (pressed,
	 * released) and tooltip.
	 *
	 * @param MARKERS
	 */
	...
	// ******************** UI related methods ********************************
	/**
	 * A convenient method to set the color of foreground elements like title,
	 * subTitle, unit, value, tickLabel and tickMark to the given Color.
	 *
	 * @param COLOR
	 */
	...
	public boolean isStartFromZero() {
		return null == startFromZero ? _startFromZero : startFromZero.get();
	}
	...
	public BooleanProperty startFromZeroProperty() {
		if (null == startFromZero) {
			startFromZero = new BooleanPropertyBase(_startFromZero) {
				@Override
				protected void invalidated() {
					// Gauge.this.setValue((get() && getMinValue() < 0) ? 0 :
					// getMinValue());
					fireUpdateEvent(REDRAW_EVENT);
				}
	...
	public boolean isReturnToZero() {
		return null == returnToZero ? _returnToZero : returnToZero.get();
	}

	/**
	 * Defines the behavior of the visualization where the needle/bar should
	 * always return to 0 after it reached the final value. This setting only
	 * makes sense if animated == true and the data rate is not too high. Set to
	 * false when using real measured live data.
	 *
	 * @param IS_TRUE
	 */
	...
	public BooleanProperty returnToZeroProperty() {
		if (null == returnToZero) {
			returnToZero = new BooleanPropertyBase(_returnToZero) {
				@Override
				protected void invalidated() {
					if (Double.compare(getMaxValue(), 0.0) > 0)
						set(false);
					fireUpdateEvent(REDRAW_EVENT);
				}
	...
	/**
	 * Returns the color that will be used to colorize the 0 tickmark and
	 * ticklabel when the gauge range has a negative min- and positive maxValue.
	 *
	 * @return the color that will used to visualize the 0 tickmark and
	 *         ticklabel
	 */
	public Color getZeroColor() {
		return null == zeroColor ? _zeroColor : zeroColor.get();
	}

	/**
	 * Defines the color that will be used to colorize the 0 tickmark and
	 * ticklabel when the gauge range has a negative min- and positive maxValue.
	 *
	 * @param COLOR
	 */
	...
	/**
	 * Returns the smallest value that was measured after the last reset. The
	 * default value is the maxValue of the gauge.
	 *
	...
	/**
	 * Returns the biggest value that was measured after the last reset. The
	 * default value is the minValue of the gauge.
	 *
	...
	public boolean isOldValueVisible() {
		return null == oldValueVisible ? _oldValueVisible : oldValueVisible.get();
	}
	...
	/**
	 * Returns true if the visualization of the gauge value is visible. Usually
	 * this is a Label or Text node.
	 *
	...
	public Paint getBackgroundPaint() {
		return null == backgroundPaint ? _backgroundPaint : backgroundPaint.get();
	}
	...
	/**
	 * Returns the Paint object that will be used to draw the border of the
	 * gauge. Usually this is a Color object.
	 *
	 * @return the Paint object that will be used to draw the border of the
	 *         gauge
	 */
	public Paint getBorderPaint() {
		return null == borderPaint ? _borderPaint : borderPaint.get();
	}

	/**
	 * Defines the Paint object that will be used to draw the border of the
	 * gauge.
	 *
	 * @param PAINT
	 */
	...
	/**
	 * Returns the width in pixels that will be used to draw the border of the
	 * gauge. The value will be clamped between 0 and 50 pixels.
	 *
	 * @return the width in pixels that will be used to draw the border of the
	 *         gauge
	 */
	public double getBorderWidth() {
		return null == borderWidth ? _borderWidth : borderWidth.get();
	}

	/**
	 * Defines the width in pixels that will be used to draw the border of the
	 * gauge. The value will be clamped between 0 and 50 pixels.
	 *
	 * @param WIDTH
	 */
		...
		if (null == borderWidth) {
			borderWidth = new DoublePropertyBase(_borderWidth) {
				...
				protected void invalidated() {
					...
					if (WIDTH < 0 || WIDTH > 50)
						set(Helper.clamp(0.0, 50.0, WIDTH));
					fireUpdateEvent(REDRAW_EVENT);
				}
	...
	/**
	 * Returns the Paint object that will be used to fill the foreground of the
	 * gauge. This could be used to visualize glass effects etc. and is only
	 * rarely used.
	 *
	 * @return the Paint object that will be used to fill the foreground of the
	 *         gauge
	 */
	public Paint getForegroundPaint() {
		return null == foregroundPaint ? _foregroundPaint : foregroundPaint.get();
	}

	/**
	 * Defines the Paint object that will be used to fill the foreground of the
	 * gauge. This could be used to visualize glass effects etc. and is only
	 * rarely used.
	 *
	 * @param PAINT
	...
	/**
	 * Returns the color that will be used to colorize the knob of the radial
	 * gauges.
	 *
	 * @return the color that will be used to colorize the knob of the radial
	 *         gauges
	 */
	public Color getKnobColor() {
		return null == knobColor ? _knobColor : knobColor.get();
	}

	/**
	 * Defines the color that will be used to colorize the knob of the radial
	 * gauges.
	 *
	 * @param COLOR
	 */
	...
	/**
	 * Returns the type of knob that will be used in the radial gauges. The
	 * values are STANDARD, PLAIN, METAL and FLAT.
	 *
	 * @return the type of knob that will be used in the radial gauges
	 */
	public KnobType getKnobType() {
		return null == knobType ? _knobType : knobType.get();
	}

	/**
	 * Defines the type of knob that will be used in the radial gauges. The
	 * values are STANDARD, PLAIN, METAL and FLAT.
	 *
	 * @param TYPE
	 */
	...
	/**
	 * Returns the position of the knob in radial gauges. This position also
	 * defines where the needle will be placed. Dependent on the SkinType you
	 * can use the following values GaugeSkin : CENTER HSkin : TOP_CENTER,
	 * BOTTOM_CENTER VSkin : CENTER_LEFT, CENTER_RIGHT QuarterSkin: TOP_RIGHT,
	 * BOTTOM_RIGHT, BOTTOM_LEFT, TOP_LEFT
	 *
	 * @return the position of the knob in the radial gauges
	 */
	public Pos getKnobPosition() {
		return null == knobPosition ? _knobPosition : knobPosition.get();
	}

	/**
	 * Defines the position of the knob in radial gauges. This position also
	 * defines where the needle will be placed. Dependent on the SkinType you
	 * can use the following values GaugeSkin : CENTER HSkin : TOP_CENTER,
	 * BOTTOM_CENTER VSkin : CENTER_LEFT, CENTER_RIGHT QuarterSkin: TOP_RIGHT,
	 * BOTTOM_RIGHT, BOTTOM_LEFT, TOP_LEFT
	...
	public boolean isKnobVisible() {
		return null == knobVisible ? _knobVisible : knobVisible.get();
	}
	...
	/**
	 * Returns true if setting the value of the gauge will be animated using the
	 * duration defined in animationDuration [ms]. Keep in mind that it only
	 * makes sense to animate the setting if the data rate is low (more than 1
	 * value per second). If you use real live measured data you should set
	 * animated to false.
	 *
	...
	/**
	 * Defines if setting the value of the gauge should be animated using the
	 * duration defined in animationDuration [ms]. Keep in mind that it only
	 * makes sense to animate the setting if the data rate is low (more than 1
	 * value per second). If you use real live measured data you should set
	 * animated to false.
	...
	public BooleanProperty animatedProperty() {
		if (null == animated) {
			animated = new SimpleBooleanProperty(Gauge.this, "animated", _animated);
		}
		return animated;
	}

	/**
	 * Returns the duration in milliseconds that will be used to animate the
	 * needle/bar of the gauge from the last value to the next value. This will
	 * only be used if animated == true. This value will be clamped in the range
	 * of 10ms - 10s.
	 *
	...
	/**
	 * Defines the duration ín milliseconds that will be used to animate the
	 * needle/bar of the gauge from the last value to the next value. This will
	 * only be used if animated == true. This value will be clamped in the range
	 * of 10ms - 10s.
	 *
	...
	public void setAnimationDuration(final long ANIMATION_DURATION) {
		animationDuration = Helper.clamp(10, 10000, ANIMATION_DURATION);
	}

	/**
	 * Returns the angle in degree that defines the start of the scale with it's
	 * minValue in a radial gauge. If set to 0 the scale will start at the
	 * bottom center and the direction of counting is mathematical correct
	 * counter-clockwise. Means if you would like to start the scale on the left
	 * side in the middle of the gauge height the startAngle should be set to
	 * 270 degrees.
	 *
	...
	/**
	 * Defines the angle in degree that defines the start of the scale with it's
	 * minValue in a radial gauge. If set to 0 the scale will start at the
	 * bottom center and the direction of counting is mathematical correct
	 * counter-clockwise. Means if you would like to start the scale on the left
	 * side in the middle of the gauge height the startAngle should be set to
			...
			startAngle = new DoublePropertyBase(_startAngle) {
				...
				protected void invalidated() {
					...
					if (ANGLE < 0 || ANGLE > 360)
						set(Helper.clamp(0.0, 360.0, ANGLE));
					fireUpdateEvent(RECALC_EVENT);
				}
	...
	/**
	 * Returns the angle range in degree that will be used to draw the scale of
	 * the radial gauge. The given range will be clamped in the range of 0 - 360
	 * degrees and will be drawn in the direction dependent on the
	 * scaleDirection.
	 *
	...
	/**
	 * Defines the angle range in degree that will be used to draw the scale of
	 * the radial gauge. The given range will be clamped in the range of 0 - 360
	 * degrees. The range will start at the startAngle and will be drawn in the
	 * direction dependent on the scaleDirection.
	 *
	 * @param RANGE
	 */
	...
	/**
	 * Returns the value that is calculated by dividing the angleRange by the
	 * range. The angleStep will always be recalculated when changing the min-,
	 * maxValue or angleRange. E.g. angleRange = 180 degrees, range = 0 - 100
	 * will lead to angleStep = 180/100 = 1.8
	 *
	 * @return the value that is calculated by dividing the angleRange by the
	 *         range
	 */
	public double getAngleStep() {
		return null == angleStep ? _angleStep : angleStep.get();
	}
	...
	public ReadOnlyDoubleProperty angleStepProperty() {
		if (null == angleStep) {
			angleStep = new SimpleDoubleProperty(Gauge.this, "angleStep", _angleStep);
		}
		return angleStep;
	}

	/**
	 * Returns true if the scale will be calculated automatically based on the
	 * defined values for min- and maxValue. The autoscaling is on per default
	 * because otherwise you will run into problems when having very large or
	 * very small scales like 0 - 10000 or 0 - 1.
	 *
	...
	/**
	 * Defines if the scale should be calculated automatically based on the
	 * defined values for min- and maxValue. The autoscaling is on per default
	 * because otherwise you will run into problems when having very large or
	 * very small scales like 0 - 10000 or 0 - 1.
	 *
	...
	public void setAutoScale(final boolean AUTO_SCALE) {
		if (null == autoScale) {
			...
			if (_autoScale) {
				...
				originalMaxValue = getMaxValue();
				setMinValue(
						Double.compare(-Double.MAX_VALUE, originalMinValue) == 0 ? getMinValue() : originalMinValue);
	...
	/**
	 * Returns true if effects like shadows will be drawn. In some gauges inner-
	 * and dropshadows will be used which will be switched on/off by setting the
	 * shadowsEnabled property.
	 *
	 * @return true if effects like shadows will be drawn
	 */
	public boolean isShadowsEnabled() {
		return null == shadowsEnabled ? _shadowsEnabled : shadowsEnabled.get();
	}

	/**
	 * Defines if effects like shadows should be drawn. In some gauges inner-
	 * and dropshadows will be used which will be switched on/off by setting the
	 * shadowsEnabled property.
	 *
	 * @param ENABLED
	...
	/**
	 * Returns true if the highlight effect on the gauges like the LinearSkin
	 * bar will be drawn. If you would like to have a more flat style you should
	 * set this to false.
	 *
	 * @return true if the highlight effect on a bar will be drawn
	 */
	public boolean isBarEffectEnabled() {
		return null == barEffectEnabled ? _barEffectEnabled : barEffectEnabled.get();
	}

	/**
	 * Defines if the the highlight effect on the gauges like the LinearSkin bar
	 * will be drawn. If you would like to have a more flat style you should set
	 * this to false.
	 *
	 * @param ENABLED
	 */
	...
	/**
	 * Returns the direction of the scale. The values are CLOCKWISE and
	 * COUNTER_CLOCKWISE. This property is needed to realize gauges like in
	 * QuarterSkin where the needle and knob should be placed on the upper right
	 * corner and the scale should start at the bottom. Here you need a
	 * counter-clockwise direction of the scale.
	 *
	 * @return the direction of the scale
	 */
	public ScaleDirection getScaleDirection() {
		return null == scaleDirection ? _scaleDirection : scaleDirection.get();
	}

	/**
	 * Defines the direction of the scale. The values are CLOCKWISE and
	 * COUNTER_CLOCKWISE. This property is needed to realize gauges like in
	 * QuarterSkin where the needle and knob should be placed on the upper right
	 * corner and the scale should start at the bottom. Here you need a
	 * counter-clockwise direction of the scale.
	...
	/**
	 * Returns the location of the ticklabels. The values are INSIDE and
	 * OUTSIDE. The location of the ticklabels has an influence on the size of
	 * the tickmarks and length of the needle.
	 *
	 * @return the location of the ticklabels
	 */
	public TickLabelLocation getTickLabelLocation() {
		return null == tickLabelLocation ? _tickLabelLocation : tickLabelLocation.get();
	}

	/**
	 * Defines the location of the ticklabels. The values are INSIDE and
	 * OUTSIDE. The location of the ticklabels has an influence on the size of
	 * the tickmarks and length of the needle.
	 *
	 * @param LOCATION
	...
	/**
	 * Returns the orientation of the ticklabels. The values are HORIZONTAL,
	 * ORTHOGONAL and TANGENT. Especially the ORTHOGONAL setting can be useful
	 * when using scales with big numbers.
	 *
	...
	/**
	 * Defines the orientation of the ticklabels. The values are HORIZONTAL,
	 * ORTHOGONAL and TANGENT. Especially the ORTHOGONAL setting can be useful
	 * when using scales with big numbers.
	 *
	 * @param ORIENTATION
	...
	/**
	 * Returns the color that will be used to colorize the ticklabels. This
	 * color will only be used if no ticklabel section defines a different
	 * color.
	 *
	 * @return the color that will be used to colorize the ticklabels
	 */
	public Color getTickLabelColor() {
		return null == tickLabelColor ? _tickLabelColor : tickLabelColor.get();
	}

	/**
	 * Defines the color that will be used to colorize the ticklabels. This
	 * color will only be used if no ticklabel section defines a different
	 * color.
	 *
	 * @param COLOR
	 ...
	 * will only be used if no tickmark section or major-, medium- and
	 * minorTickMarkColor is defined at the position of the tickmark.
	 *
	 * @return the color that will be used to colorize the tickmarks
	 */
	public Color getTickMarkColor() {
		return null == tickMarkColor ? _tickMarkColor : tickMarkColor.get();
	}
	...
	/**
	 * Returns the color that will be used to colorize the major tickmarks. This
	 * color will only be used if no tickmark section is defined at the position
	 * of the tickmark.
	 *
	 * @return the color that will be used to colorize the major tickmarks
	 */
	public Color getMajorTickMarkColor() {
		return null == majorTickMarkColor ? _majorTickMarkColor : majorTickMarkColor.get();
	}

	/**
	 * Defines the color that will be used to colorize the major tickmarks. This
	 * color will only be used if no tickmark section is defined at the position
	 * of the tickmark.
	 *
	 * @param COLOR
	...
	/**
	 * Returns the factor that defines the length of the major tick mark. The
	 * value can be in the range from 0 - 1;
	 *
	 * @return the factor that defines the length of the major tick mark
	 */
	public double getMajorTickMarkLengthFactor() {
		return null == majorTickMarkLengthFactor ? _majorTickMarkLengthFactor : majorTickMarkLengthFactor.get();
	}

	/**
	 * The factor defines the length of the major tick mark. It can be in the
	 * range from 0 - 1.
	 *
	 * @param FACTOR
	 */
		...
		if (null == majorTickMarkLengthFactor) {
			majorTickMarkLengthFactor = new DoublePropertyBase(_majorTickMarkLengthFactor) {
				...
				protected void invalidated() {
					final double VALUE = get();
					if (VALUE < 0 || VALUE > 1)
						set(Helper.clamp(0.0, 1.0, VALUE));
					fireUpdateEvent(REDRAW_EVENT);
				}
	...
	/**
	 * Returns the factor that defines the width of the major tick mark. The
	 * value can be in the range from 0 - 1.
	 *
	 * @return the factor that defines the width of the major tick mark
	 */
	public double getMajorTickMarkWidthFactor() {
		return null == majorTickMarkWidthFactor ? _majorTickMarkWidthFactor : majorTickMarkWidthFactor.get();
	}

	/**
	 * The factor defines the width of the major tick mark. It can be in the
	 * range from 0 - 1.
	 *
	 * @param FACTOR
	 */
	 ...
	 * This color will only be used if no tickmark section is defined at the
	 * position of the tickmark.
	 *
	 * @return the color that will be used to colorize the medium tickmark
	 */
	public Color getMediumTickMarkColor() {
		return null == mediumTickMarkColor ? _mediumTickMarkColor : mediumTickMarkColor.get();
	}
	...
	/**
	 * Returns the factor that defines the length of the medium tick mark. The
	 * value can be in the range from 0 - 1;
	 *
	 * @return the factor that defines the length of the medium tick mark
	 */
	public double getMediumTickMarkLengthFactor() {
		return null == mediumTickMarkLengthFactor ? _mediumTickMarkLengthFactor : mediumTickMarkLengthFactor.get();
	}

	/**
	 * The factor defines the length of the medium tick mark. It can be in the
	 * range from 0 - 1.
	 *
	 * @param FACTOR
	 */
	...
	/**
	 * Returns the factor that defines the width of the medium tick mark. The
	 * value can be in the range from 0 - 1.
	 *
	 * @return the factor that defines the width of the medium tick mark
	 */
	public double getMediumTickMarkWidthFactor() {
		return null == mediumTickMarkWidthFactor ? _mediumTickMarkWidthFactor : mediumTickMarkWidthFactor.get();
	}

	/**
	 * The factor defines the width of the medium tick mark. It can be in the
	 * range from 0 - 1.
	 *
	 * @param FACTOR
	 */
	...
	/**
	 * Returns the color that will be used to colorize the minor tickmarks. This
	 * color will only be used if no tickmark section is defined at the position
	 * of the tickmark.
	 *
	...
	public Color getMinorTickMarkColor() {
		return null == minorTickMarkColor ? _minorTickMarkColor : minorTickMarkColor.get();
	}

	/**
	 * Defines the color that will be used to colorize the minor tickmarks. This
	 * color will only be used if no tickmark section is defined at the position
	 * of the tickmark.
	 *
	 * @param COLOR
	...
	/**
	 * Returns the factor that defines the length of the minor tick mark. The
	 * value can be in the range from 0 - 1;
	 *
	 * @return the factor that defines the length of the minor tick mark
	 */
	public double getMinorTickMarkLengthFactor() {
		return null == minorTickMarkLengthFactor ? _minorTickMarkLengthFactor : minorTickMarkLengthFactor.get();
	}

	/**
	 * The factor defines the length of the minor tick mark. It can be in the
	 * range from 0 - 1.
	 *
	 * @param FACTOR
	 */
	...
	/**
	 * Returns the factor that defines the width of the minor tick mark. The
	 * value can be in the range from 0 - 1.
	 *
	 * @return the factor that defines the width of the minor tick mark
	 */
	public double getMinorTickMarkWidthFactor() {
		return null == minorTickMarkWidthFactor ? _minorTickMarkWidthFactor : minorTickMarkWidthFactor.get();
	}

	/**
	 * The factor defines the width of the minor tick mark. It can be in the
	 * range from 0 - 1.
	 *
	 * @param FACTOR
	 */
	...
	public TickMarkType getMajorTickMarkType() {
		return null == majorTickMarkType ? _majorTickMarkType : majorTickMarkType.get();
	}
	...
	public TickMarkType getMediumTickMarkType() {
		return null == mediumTickMarkType ? _mediumTickMarkType : mediumTickMarkType.get();
	}
	...
	public TickMarkType getMinorTickMarkType() {
		return null == minorTickMarkType ? _minorTickMarkType : minorTickMarkType.get();
	}
	...
	/**
	 * Returns the number format that will be used to format the value in the
	 * gauge (NOT USED AT THE MOMENT)
	 *
	...
	/**
	 * Defines the number format that will be used to format the value in the
	 * gauge (NOT USED AT THE MOMENT)
	 *
	 * @param FORMAT
	 */
	...
	public ObjectProperty<NumberFormat> numberFormatProperty() {
		if (null == numberFormat) {
			numberFormat = new ObjectPropertyBase<NumberFormat>(_numberFormat) {
				...
				protected void invalidated() {
					if (null == get())
						set(NumberFormat.getInstance(getLocale()));
					fireUpdateEvent(RESIZE_EVENT);
				}
	...
	/**
	 * Returns the number of decimals that will be used to format the value of
	 * the gauge. The number of decimals will be clamped to a value between 0-3.
	 *
	...
	/**
	 * Defines the number of decimals that will be used to format the value of
	 * the gauge. The number of decimals will be clamped to a value between 0-3.
	 *
	 * @param DECIMALS
	 */
	...
	public IntegerProperty decimalsProperty() {
		if (null == decimals) {
			decimals = new IntegerPropertyBase(_decimals) {
				...
				protected void invalidated() {
					final int VALUE = get();
					if (VALUE < 0 || VALUE > MAX_NO_OF_DECIMALS)
						set(Helper.clamp(0, MAX_NO_OF_DECIMALS, VALUE));
					fireUpdateEvent(REDRAW_EVENT);
				}
	...
	/**
	 * Returns the number of decimals that will be used to format the value of
	 * the ticklabels. The number of decimals will be clamped to a value between
	 * 0-3.
	 *
	 * @return
	 */
	public int getTickLabelDecimals() {
		return null == tickLabelDecimals ? _tickLabelDecimals : tickLabelDecimals.get();
	}
	...
	/**
	 * Returns the shape of the needle that will be used. This is dependent on
	 * the used SkinType. Values are GaugeSkin : STANDARD, FAT HSkin : STANDARD
	 * VSkin : STANDARD QuarterSkin: STANDARD
	 *
	...
	/**
	 * Defines the shape of the needle that will be used. This is dependent on
	 * the used SkinType. Values are GaugeSkin : STANDARD, FAT HSkin : STANDARD
	 * VSkin : STANDARD QuarterSkin: STANDARD
	 *
	 * @param TYPE
	 ...
	 * Values are ANGLED, ROUND and FLAT In principle it defines how the needle
	 * will be filled (gradient, color)
	 *
	 * @return the graphical representation of the needle
	 */
	public NeedleShape getNeedleShape() {
		return null == needleShape ? _needleShape : needleShape.get();
	}
	...
	/**
	 * Returns the thickness of the needle. The values are THIN, STANDARD and
	 * THICK
	 *
	 * @return the thickness of the needle
	 */
	public NeedleSize getNeedleSize() {
		return null == needleSize ? _needleSize : needleSize.get();
	}

	/**
	 * Defines the thickness of the needle. The values are THIN, STANDARD and
	 * THICK
	 *
	 * @param SIZE
	 */
	...
	/**
	 * Returns the behavior of the needle movement. The values are STANDARD and
	 * OPTIMIZED This is an experimental feature that only makes sense in gauges
	 * that use an angleRange of 360 degrees and where the needle should be able
	 * to use the shortest way to the target value. As an example one can think
	 * of a compass. If the value in a compass changes from 20 degrees to 290
	 * degrees the needle will take the shortest way to the value, in this case
	 * this means it will rotate counter-clockwise.
	 *
	 * @return the behavior of the needle movement (EXPERIMENTAL)
	 */
	public NeedleBehavior getNeedleBehavior() {
		return null == needleBehavior ? _needleBehavior : needleBehavior.get();
	}

	/**
	 * Defines the behavior of the needle movement. The values are STANDARD and
	 * OPTIMIZED This is an experimental feature that only makes sense in gauges
	 * that use an angleRange of 360 degrees and where the needle should be able
	 * to use the shortest way to the target value. As an example one can think
	 * of a compass. If the value in a compass changes from 20 degrees to 290
	...
	/**
	 * Returns the color that will be used to colorize the needle of the radial
	 * gauges.
	 *
	...
	/**
	 * Defines the color that will be used to colorize the needle of the radial
	 * gauges.
	 *
	 * @param COLOR
	 */
	...
	public Color getNeedleBorderColor() {
		return null == needleBorderColor ? _needleBorderColor : needleBorderColor.get();
	}
	...
	/**
	 * Returns the color that will be used to colorize the bar of the gauge (if
	 * it has a bar).
	 *
	...
	/**
	 * Defines the color that will be used to colorize the bar of the gauge (if
	 * it has a bar).
	 *
	 * @param COLOR
	 */
	...
	public Color getBarBorderColor() {
		return null == barBorderColor ? _barBorderColor : barBorderColor.get();
	}
	...
	/**
	 * Returns the color that will be used to colorize the bar background of the
	 * gauge (if it has a bar).
	 *
	 * @return the color that will be used to colorize the bar background
	 */
	public Color getBarBackgroundColor() {
		return null == barBackgroundColor ? _barBackgroundColor : barBackgroundColor.get();
	}
	...
	/**
	 * Returns the design that will be used to visualize the LCD display of the
	 * gauge (if it has one). The values are BEIGE, BLACK, BLUE, ORANGE, RED,
	 * YELLOW, WHITE, GRAY, BLACK, GREEN, GREEN_DARKGREEN, BLUE2, BLUE_BLACK,
	 * BLUE_DARKBLUE, BLUE_LIGHTBLUE, BLUE_GRAY, STANDARD, LIGHTGREEN,
	 * STANDARD_GREEN, BLUE_BLUE, RED_DARKRED, DARKBLUE, PURPLE, BLACK_RED,
	 * DARKGREEN, AMBER, LIGHTBLUE, GREEN_BLACK, YELLOW_BLACK, BLACK_YELLOW,
	 * LIGHTGREEN_BLACK, DARKPURPLE, DARKAMBER, BLUE_LIGHTBLUE2, GRAY_PURPLE,
	 * YOCTOPUCE, SECTIONS, FLAT_CUSTOM
	 *
	 * @return the design that will be used to visualize the LCD display (if
	 *         available)
	 */
	public LcdDesign getLcdDesign() {
		return null == lcdDesign ? _lcdDesign : lcdDesign.get();
	}

	/**
	 * Defines the design that will be used to visualize the LCD display of the
	 * gauge (if it has one). The values are BEIGE, BLACK, BLUE, ORANGE, RED,
	 * YELLOW, WHITE, GRAY, BLACK, GREEN, GREEN_DARKGREEN, BLUE2, BLUE_BLACK,
	 * BLUE_DARKBLUE, BLUE_LIGHTBLUE, BLUE_GRAY, STANDARD, LIGHTGREEN,
	 * STANDARD_GREEN, BLUE_BLUE, RED_DARKRED, DARKBLUE, PURPLE, BLACK_RED,
	...
	/**
	 * Returns the font that will be used to visualize the LCD value if the
	 * gauge has a LCD display. The values are STANDARD, LCD, SLIM,
	 * DIGITAL_BOLD, ELEKTRA
	 *
	 * @return the font that will be used to visualize the LCD value
	 */
	public LcdFont getLcdFont() {
		return null == lcdFont ? _lcdFont : lcdFont.get();
	}

	/**
	 * Defines the font that will be used to visualize the LCD value if the
	 * gauge has a LCD display. The values are STANDARD, LCD, SLIM,
	 * DIGITAL_BOLD, ELEKTRA
	 *
	 * @param FONT
	...
	/**
	 * Returns the color that will be used to visualize the LED of the gauge if
	 * it has one.
	 *
	...
	/**
	 * Defines the color that will be used to visualize the LED of the gauge if
	 * it has one.
	 *
	 * @param COLOR
	 */
	...
	/**
	 * Returns the graphical representation of the LED. The values are STANDARD
	 * and FLAT In principle this represents how the LED will be filled
	 * (gradient or color).
	 *
	...
	/**
	 * Defines the graphical representation of the LED. The values are STANDARD
	 * and FLAT In principle this represents how the LED will be filled
	 * (gradient or color).
	 *
	 * @param TYPE
	...
	/**
	 * Returns the color that will be used to colorize the subTitle of the
	 * gauge.
	 *
	 * @return the color that will be used to colorize the subTitle
	 */
	public Color getSubTitleColor() {
		return null == subTitleColor ? _subTitleColor : subTitleColor.get();
	}

	/**
	 * Defines the color that will be used to colorize the subTitle of the
	 * gauge.
	 *
	 * @param COLOR
	 */
	...
	public Color getUnitColor() {
		return null == unitColor ? _unitColor : unitColor.get();
	}
	...
	public Color getValueColor() {
		return null == valueColor ? _valueColor : valueColor.get();
	}
	...
	/**
	 * Returns the color that will be used to colorize the threshold indicator
	 * of the gauge.
	 *
	 * @return the color that will be used to colorize the threshold indicator
	 */
	public Color getThresholdColor() {
		return null == thresholdColor ? _thresholdColor : thresholdColor.get();
	}

	/**
	 * Defines the color that will be used to colorize the threshold indicator
	 * of the gauge.
	 *
	 * @param COLOR
	 */
	...
	/**
	 * Returns the color that will be used to colorize the average indicator of
	 * the gauge.
	 *
	 * @return the color that will be used to colorize the average indicator
	 */
	public Color getAverageColor() {
		return null == averageColor ? _averageColor : averageColor.get();
	}

	/**
	 * Defines the color that will be used to colorize the average indicator of
	 * the gauge.
	 *
	 * @param COLOR
	 */
	...
	/**
	 * Returns true if the value of the gauge should be checked against all
	 * sections (if sections not empty). If a value enters a section or leaves a
	 * section it will fire an event. The check will be performed after the
	 * animation is finished (if animated == true).
	 *
	 * @return true if the value of the gauge should be checked against all
	 *         sections
	 */
	public boolean getCheckSectionsForValue() {
		return null == checkSectionsForValue ? _checkSectionsForValue : checkSectionsForValue.get();
	}

	/**
	 * Defines if the value of the gauge should be checked against all sections
	 * (if sections not empty). If a value enters a section or leaves a section
	 * it will fire an event. The check will be performed after the animation is
	 * finished (if animated == true).
	 *
	 * @param CHECK
	 */
	...
	public BooleanProperty checkSectionsForValueProperty() {
		if (null == checkSectionsForValue) {
			checkSectionsForValue = new SimpleBooleanProperty(Gauge.this, "checkSectionsForValue",
					_checkSectionsForValue);
		}
	...
	/**
	 * Returns true if the value of the gauge should be checked against all
	 * areas (if areas not empty). If a value enters an area or leaves an area
	 * it will fire an event. The check will be performed after the animation is
	 * finished (if animated == true).
	 *
	 * @return true if the the value of the gauge should be checked against all
	 *         areas
	 */
	public boolean getCheckAreasForValue() {
		return null == checkAreasForValue ? _checkAreasForValue : checkAreasForValue.get();
	}

	/**
	 * Defines if the value of the gauge should be checked against all areas (if
	 * areas not empty). If a value enters an area or leaves an area it will
	 * fire an event. The check will be performed after the animation is
	 * finished (if animated == true).
	 *
	 * @param CHECK
	 */
	...
	public BooleanProperty checkAreasForValueProperty() {
		if (null == checkAreasForValue) {
			checkAreasForValue = new SimpleBooleanProperty(Gauge.this, "checkAreasForValue", _checkAreasForValue);
		}
		return checkAreasForValue;
	}

	/**
	 * Returns true if the value of the gauge should be checked against the
	 * threshold. If a value crosses the threshold it will fire an event
	 * (EXCEEDED and UNDERRUN. The check will be performed after the animation
	 * is finished (if animated == true).
	 *
	 * @return true if the value of the gauge should be checked against the
	 *         threshold
	 */
	public boolean isCheckThreshold() {
		return null == checkThreshold ? _checkThreshold : checkThreshold.get();
	}

	/**
	 * Defines if the value of the gauge should be checked against the
	 * threshold. If a value crosses the threshold it will fire an event
	 * (EXCEEDED and UNDERRUN. The check will be performed after the animation
	 * is finished (if animated == true).
	 *
	...
	public BooleanProperty checkThresholdProperty() {
		if (null == checkThreshold) {
			checkThreshold = new SimpleBooleanProperty(Gauge.this, "checkThreshold", _checkThreshold);
		}
	...
	public boolean isInnerShadowEnabled() {
		return null == innerShadowEnabled ? _innerShadowEnabled : innerShadowEnabled.get();
	}
	...
	public boolean isAverageVisible() {
		return null == averageVisible ? _averageVisible : averageVisible.get();
	}
	...
	public boolean getSectionsAlwaysVisible() {
		return null == sectionsAlwaysVisible ? _sectionsAlwaysVisible : sectionsAlwaysVisible.get();
	}

	/**
	 * Defines if the sections will always be visible. This is currently only
	 * used in the IndicatorSkin
	 * 
	 * @param VISIBLE
	 */
	...
	/**
	 * Returns true if the text of the sections should be drawn inside the
	 * sections. This is currently only used in the SimpleSkin.
	 *
	 ...
	 * This is currently only used in the SimpleSkin.
	 *
	 * @param VISIBLE
	 */
	...
	/**
	 * Returns true if the icon of the sections should be drawn inside the
	 * sections. This is currently only used in the SimpleSkin.
	 *
	 * @return true if the icon of the sections should be drawn
	 */
	public boolean getSectionIconsVisible() {
		return null == sectionIconsVisible ? _sectionIconsVisible : sectionIconsVisible.get();
	}
	...
	/**
	 * Returns true if sections should be highlighted in case they contain the
	 * current value.
	 *
	 * @return true if sections should be highlighted
	 */
	public boolean isHighlightSections() {
		return null == highlightSections ? _highlightSections : highlightSections.get();
	}

	/**
	 * Defines if sections should be highlighted in case they contain the
	 * current value
	 *
	 * @param HIGHLIGHT
	 */
	...
	public boolean isAreaTextVisible() {
		return null == areaTextVisible ? _areaTextVisible : areaTextVisible.get();
	}
	...
	public boolean getAreaIconsVisible() {
		return null == areaIconsVisible ? _areaIconsVisible : areaIconsVisible.get();
	}
	...
	/**
	 * Returns true if areas should be highlighted in case they contain the
	 * current value.
	 *
	 * @return true if areas should be highlighted
	 */
	public boolean isHighlightAreas() {
		return null == highlightAreas ? _highlightAreas : highlightAreas.get();
	}

	/**
	 * Defines if areas should be highlighted in case they contain the current
	 * value
	 *
	 * @param HIGHLIGHT
	 */
	...
	/**
	 * Returns true if the tickmark sections should be used to colorize the
	 * tickmarks.
	 *
	 * @return true if the tickmark sections should be used
	 */
	public boolean getTickMarkSectionsVisible() {
		return null == tickMarkSectionsVisible ? _tickMarkSectionsVisible : tickMarkSectionsVisible.get();
	}

	/**
	 * Defines if the tickmark sections should be used to colorize the
	 * tickmarks.
	 *
	 * @param VISIBLE
	 */
	...
	/**
	 * Returns true if the ticklabel sections should be used to colorize the
	 * ticklabels.
	 *
	 * @return true if the ticklabel sections should be used
	 */
	public boolean getTickLabelSectionsVisible() {
		return null == tickLabelSectionsVisible ? _tickLabelSectionsVisible : tickLabelSectionsVisible.get();
	}

	/**
	 * Defines if the ticklabel sections should be used to colorize the
	 * ticklabels.
	 *
	 * @param VISIBLE
	 */
	...
	public boolean getTickLabelsVisible() {
		return null == tickLabelsVisible ? _tickLabelsVisible : tickLabelsVisible.get();
	}
	 ...
	 * Sometimes this could be useful if a gauge should for example only should
	 * show 0 and 1000.
	 *
	 * @return true if only the first and last ticklabel will be drawn
	 */
	public boolean isOnlyFirstAndLastTickLabelVisible() {
		return null == onlyFirstAndLastTickLabelVisible ? _onlyFirstAndLastTickLabelVisible
				: onlyFirstAndLastTickLabelVisible.get();
	}

	/**
	 * Defines if only the first and the last ticklabel will be drawn. Sometimes
	 * this could be useful if a gauge should for example only should show 0 and
	 * 1000.
	 *
	 * @param VISIBLE
	 */
	...
	/**
	 * Returns true if the major tickmarks should be drawn If set to false and
	 * minorTickmarks == true, a minor tickmark will be drawn instead of the
	 * major tickmark.
	 *
	 * @return true if the major tickmarks should be drawn
	 */
	public boolean getMajorTickMarksVisible() {
		return null == majorTickMarksVisible ? _majorTickMarksVisible : majorTickMarksVisible.get();
	}

	/**
	 * Defines if the major tickmarks should be drawn If set to false and
	 * minorTickmarks == true, a minor tickmark will be drawn instead of the
	 * major tickmark.
	 *
	 * @param VISIBLE
	...
	/**
	 * Returns true if the medium tickmarks should be drawn If set to false and
	 * minorTickmarks == true, a minor tickmark will be drawn instead of the
	 * medium tickmark.
	 *
	...
	public boolean getMediumTickMarksVisible() {
		return null == mediumTickMarksVisible ? _mediumTickMarksVisible : mediumTickMarksVisible.get();
	}
	...
	public boolean getMinorTickMarksVisible() {
		return null == minorTickMarksVisible ? _minorTickMarksVisible : minorTickMarksVisible.get();
	}
	 ...
	 * tick marks.
	 *
	 * @return true when an additional ring will be drawn that "connects" the
	 *         tick marks
	 */
	public boolean isTickMarkRingVisible() {
		return null == tickMarkRingVisible ? _tickMarkRingVisible : tickMarkRingVisible.get();
	}

	/**
	 * Defines if an additional ring should be drawn that "connects" the tick
	 * marks.
	 *
	 * @param VISIBLE
	 */
	...
	/**
	 * Returns the value that represents the space between major tickmarks. This
	 * value will be automatically set by the autoscale property. Be careful
	 * when changing it manually.
	 *
	...
	/**
	 * Defines the value that represents the space between major tickmarks. This
	 * value will be automatically set by the autoscale property. Be careful
	 * when changing it manually.
	 *
	 * @param SPACE
	...
	/**
	 * Returns the value that represents the space between minor tickmarks. This
	 * value will be automatically set by the autoscale property. Be careful
	 * when changing it manually.
	 *
	...
	/**
	 * Returns true if the LCD display is visible (if available) The LCD display
	 * won't be visible if valueVisible == false.
	 *
	 * @return true if the LCD display is visible
	 */
	public boolean isLcdVisible() {
		return null == lcdVisible ? _lcdVisible : lcdVisible.get();
	}

	/**
	 * Defines if the LCD display is visible (if available) The LCD display
	 * won't be visible if valueVisible == false.
	 *
	 * @param VISIBLE
	 */
	...
	/**
	 * Returns true if the crystal effect of the LCD display will be drawn. This
	 * feature could decrease the performance if you run it on embedded devices
	 * because it will calculate a bitmap image where each pixel will be
	 * calculated.
	 *
	 * @return true if the crystal effect of the LCD display will be drawn
	 */
	public boolean isLcdCrystalEnabled() {
		return null == lcdCrystalEnabled ? _lcdCrystalEnabled : lcdCrystalEnabled.get();
	}

	/**
	 * Defines if the crystal effect of the LCD display will be drawn. This
	 * feature could decrease the performance if you run it on embedded devices
	 * because it will calculate a bitmap image where each pixel will be
	 * calculated.
	 *
	...
	public boolean isLedVisible() {
		return null == ledVisible ? _ledVisible : ledVisible.get();
	}
	...
	public boolean isLedOn() {
		return null == ledOn ? _ledOn : ledOn.get();
	}
	...
	public boolean isLedBlinking() {
		return null == ledBlinking ? _ledBlinking : ledBlinking.get();
	}
	...
	/**
	 * Returns the orientation of the control. This feature will only be used in
	 * the BulletChartSkin and LinearSkin. Values are HORIZONTAL and VERTICAL
	 *
	...
	/**
	 * Defines the orientation of the control. This feature will only be used in
	 * the BulletChartSkin and LinearSkin. Values are HORIZONTAL and VERTICAL
	 *
	 * @param ORIENTATION
	 */
	...
	/**
	 * Returns true if the gradient defined by the gradient lookup will be used
	 * to visualize the bar (if available).
	 *
	 * @return true if the gradient defined by the gradient lookup will be used
	 *         to visualize the bar
	 */
	public boolean isGradientBarEnabled() {
		return null == gradientBarEnabled ? _gradientBarEnabled : gradientBarEnabled.get();
	}

	/**
	 * Defines if the gradient defined by the gradient lookup will be used to
	 * visualize the bar (if available).
	 *
	 * @param ENABLED
	 */
	...
	/**
	 * Defines the GradientLookup that is used to colorize the bar of the gauge
	 * (if avaiable)
	 *
	...
	/**
	 * Returns a list of Stops that will be used to calculate the gradient in
	 * the GradientLookup.
	 *
	 * @return a list of Stops that will be used to calculate the gradient in
	 *         the GradientLookup
	 */
	...
	/**
	 * Defines a list of Stops that will be used to calculate the gradient in
	 * the GradientLookup.
	 *
	 * @param STOPS
	 */
	public void setGradientBarStops(final Stop... STOPS) {
		setGradientBarStops(Arrays.asList(STOPS));
	}
	 ...
	 * automatically calculated ones. This could be useful for gauges like a
	 * compass where you need "N", "E", "S" and "W" instead of numbers.
	 *
	 * @return true if custom ticklabels should be used
	 */
	public boolean getCustomTickLabelsEnabled() {
		return null == customTickLabelsEnabled ? _customTickLabelsEnabled : customTickLabelsEnabled.get();
	}

	/**
	 * Defines if custom ticklabels should be used instead of the automatically
	 * calculated ones. This could be useful for gauges like a compass where you
	 * need "N", "E", "S" and "W" instead of numbers.
	 *
	 * @param ENABLED
	 */
	...
	/**
	 * Returns a list of Strings that represent the ticklabels that will be used
	 * for the scale.
	 *
	 * @return a list of Strings that represent the ticklabels
	 */
	public List<String> getCustomTickLabels() {
		return customTickLabels;
	}

	/**
	 * Defines a list of Strings that represent the ticklabels that will be used
	 * for the scale.
	 *
	 * @param TICK_LABELS
	 */
	...
	public void addCustomTickLabel(final String TICK_LABEL) {
		if (null == TICK_LABEL)
			return;
		if (!customTickLabels.contains(TICK_LABEL))
			customTickLabels.add(TICK_LABEL);
		fireUpdateEvent(REDRAW_EVENT);
	}
	...
	public void removeCustomTickLabel(final String TICK_LABEL) {
		if (null == TICK_LABEL)
			return;
		if (customTickLabels.contains(TICK_LABEL))
			customTickLabels.remove(TICK_LABEL);
		fireUpdateEvent(REDRAW_EVENT);
	}
	...
	/**
	 * Returns the custom font size. The default font size is 18px at a size of
	 * 250px. This value will be used to calculate the current font size for the
	 * ticklabels when scaling.
	 *
	 * @return the custom font size
	 */
	public double getCustomTickLabelFontSize() {
		return null == customTickLabelFontSize ? _customTickLabelFontSize : customTickLabelFontSize.get();
	}

	/**
	 * Defines the custom font size. The default font size is 18px at a size of
	 * 250px. This value will be used to calculate the current font size for the
	 * ticklabels when scaling.
	 *
	 * @param SIZE
		...
		if (null == customTickLabelFontSize) {
			customTickLabelFontSize = new DoublePropertyBase(_customTickLabelFontSize) {
				...
				protected void invalidated() {
					...
					if (SIZE < 0 || SIZE > 72)
						set(Helper.clamp(0.0, 72.0, SIZE));
					fireUpdateEvent(REDRAW_EVENT);
				}
	...
	/**
	 * Returns the text that will be shown in the button tooltip. The knob in
	 * the radial gauges acts as button if interactive == true.
	 *
	 * @return the text that will be shown in the button tooltip
	 */
	public String getButtonTooltipText() {
		return null == buttonTooltipText ? _buttonTooltipText : buttonTooltipText.get();
	}

	/**
	 * Defines the text that will be shown in the button tooltip. The knob in
	 * the radial gauges acts as button if interactive == true.
	 *
	 * @param TEXT
	 */
	...
	/**
	 * Returns true if the control should keep it's aspect. This is in principle
	 * only needed if the control has different width and height.
	 *
	 * @return true if the control should keep it's aspect
	 */
	public boolean isKeepAspect() {
		return null == keepAspect ? _keepAspect : keepAspect.get();
	}

	/**
	 * Defines if the control should keep it's aspect. This is in principle only
	 * needed if the control has different width and height.
	 *
	 * @param KEEP
	 */
	...
	public BooleanProperty keepAspectProperty() {
		if (null == keepAspect) {
			keepAspect = new SimpleBooleanProperty(Gauge.this, "keepAspect", _keepAspect);
		}
		return keepAspect;
	}

	/**
	 * Returns true if the control uses the given customFont to render all text
	 * elements.
	 * 
	 * @return true if the control uses the given customFont
	 */
	public boolean isCustomFontEnabled() {
		return null == customFontEnabled ? _customFontEnabled : customFontEnabled.get();
	}

	/**
	 * Defines if the control should use the given customFont to render all text
	 * elements
	 * 
	 * @param ENABLED
	 */
	...
	/**
	 * Returns the given custom Font that can be used to render all text
	 * elements. To enable the custom font one has to set customFontEnabled =
	 * true
	 * 
	...
	/**
	 * Defines the custom font that can be used to render all text elements. To
	 * enable the custom font one has to set customFontEnabled = true
	 * 
	 * @param FONT
	 */
	...
	/**
	 * Returns true if the alert property was set. This property can be used to
	 * visualize an alert situation in a skin.
	 * 
	 * @return true if the alert property was set
	 */
	public boolean isAlert() {
		return null == alert ? _alert : alert.get();
	}

	/**
	 * Defines if the alert property should be set. This property can be used to
	 * visualize an alert situation in the skin.
	 * 
	 * @param ALERT
	 */
	...
	/**
	 * Returns the alert message text that could be used in a tooltip in case of
	 * an alert.
	 * 
	 * @return the alert message text
	 */
	public String getAlertMessage() {
		return null == alertMessage ? _alertMessage : alertMessage.get();
	}
	...
	/**
	 * Returns true when smoothing is enabled. This property is only used in the
	 * TileSparklineSkin to smooth the path. In a custom skin it could be also
	 * used for other things.
	 * 
	 * @return true when smoothing is enabled
	 */
	public boolean isSmoothing() {
		return null == smoothing ? _smoothing : smoothing.get();
	}

	/**
	 * Defines if the smoothing property should be enabled/disabled. At the
	 * moment this is only used in the TileSparklineSkin.
	 * 
	 * @param SMOOTHING
	 */
			...
			public Void call() throws Exception {
				try {
					...
					if (!Thread.currentThread().isInterrupted()) {
						// Schedule the same Callable with the current
						// updateInterval
	...
	public String toString() {
		return new StringBuilder("{").append("\"title\":").append("\"").append(getTitle()).append("\",")
				.append("\"subTitle\":").append("\"").append(getTitle()).append("\",").append("\"unit\":").append("\"")
				.append(getUnit()).append("\",").append("\"value\":").append(getValue()).append(",")
				.append("\"minValue\":").append(getMinValue()).append(",").append("\"maxValue\":").append(getMaxValue())
				.append(",").append("\"threshold\":").append(getThreshold()).append(",").append("\"minMeasuredValue\":")
				.append(getMinMeasuredValue()).append(",").append("\"maxMeasuredValue\":").append(getMaxMeasuredValue())
				.append("}").toString();
	}
	...
	protected Skin createDefaultSkin() {
		switch (skinType) {
		case AMP:
			return new AmpSkin(Gauge.this);
		case BULLET_CHART:
			return new BulletChartSkin(Gauge.this);
		case DASHBOARD:
			return new DashboardSkin(Gauge.this);
		case FLAT:
			return new FlatSkin(Gauge.this);
		case INDICATOR:
			return new IndicatorSkin(Gauge.this);
		case KPI:
			return new KpiSkin(Gauge.this);
		case MODERN:
			return new ModernSkin(Gauge.this);
		case SIMPLE:
			return new SimpleSkin(Gauge.this);
		case SLIM:
			return new SlimSkin(Gauge.this);
		case SPACE_X:
			return new SpaceXSkin(Gauge.this);
		case QUARTER:
			return new QuarterSkin(Gauge.this);
		case HORIZONTAL:
			return new HSkin(Gauge.this);
		case VERTICAL:
			return new VSkin(Gauge.this);
		case LCD:
			return new LcdSkin(Gauge.this);
		case TINY:
			return new TinySkin(Gauge.this);
		case BATTERY:
			return new BatterySkin(Gauge.this);
		case LEVEL:
			return new LevelSkin(Gauge.this);
		case LINEAR:
			return new LinearSkin(Gauge.this);
		case DIGITAL:
			return new DigitalSkin(Gauge.this);
		case SIMPLE_DIGITAL:
			return new SimpleDigitalSkin(Gauge.this);
		case SECTION:
			return new SectionSkin(Gauge.this);
		case BAR:
			return new BarSkin(Gauge.this);
		case WHITE:
			return new WhiteSkin(Gauge.this);
		case CHARGE:
			return new ChargeSkin(Gauge.this);
		case SIMPLE_SECTION:
			return new SimpleSectionSkin(Gauge.this);
		case TILE_KPI:
			return new TileKpiSkin(Gauge.this);
		case TILE_TEXT_KPI:
			return new TileTextKpiSkin(Gauge.this);
		case TILE_SPARK_LINE:
			return new TileSparklineSkin(Gauge.this);
		case GAUGE:
		default:
			return new GaugeSkin(Gauge.this);
		}
	}
	...
	public void setSkinType(final SkinType SKIN_TYPE) {
		...
		case INDICATOR:
			...
			setGradientBarStops(new Stop(0.0, Color.rgb(34, 180, 11)), new Stop(0.5, Color.rgb(255, 146, 0)),
		...
		case BAR:
			...
			setGradientBarStops(new Stop(0.0, barColor), new Stop(0.01, barColor),
	...
	// ******************** Event handling ************************************
	public void setOnUpdate(final UpdateEventListener LISTENER) {
		addUpdateEventListener(LISTENER);
	}

	public void addUpdateEventListener(final UpdateEventListener LISTENER) {
		if (!listenerList.contains(LISTENER))
			listenerList.add(LISTENER);
	}

	public void removeUpdateEventListener(final UpdateEventListener LISTENER) {
		if (listenerList.contains(LISTENER))
	...
	public void fireUpdateEvent(final UpdateEvent EVENT) {
		...
		for (int i = 0; i < listSize; i++) {
			listenerList.get(i).onUpdateEvent(EVENT);
		}
	}

	public void setOnButtonPressed(final EventHandler<ButtonEvent> HANDLER) {
		addEventHandler(ButtonEvent.BTN_PRESSED, HANDLER);
	}

	public void removeOnButtonPressed(final EventHandler<ButtonEvent> HANDLER) {
		removeEventHandler(ButtonEvent.BTN_PRESSED, HANDLER);
	}

	public void setOnButtonReleased(final EventHandler<ButtonEvent> HANDLER) {
		addEventHandler(ButtonEvent.BTN_RELEASED, HANDLER);
	}

	public void removeOnButtonReleased(final EventHandler<ButtonEvent> HANDLER) {
		removeEventHandler(ButtonEvent.BTN_RELEASED, HANDLER);
	}

	public void setOnThresholdExceeded(final EventHandler<ThresholdEvent> HANDLER) {
		addEventHandler(ThresholdEvent.THRESHOLD_EXCEEDED, HANDLER);
	}

	public void removeOnThresholdExceeded(final EventHandler<ThresholdEvent> HANDLER) {
		removeEventHandler(ThresholdEvent.THRESHOLD_EXCEEDED, HANDLER);
	}

	public void setOnThresholdUnderrun(final EventHandler<ThresholdEvent> HANDLER) {
		addEventHandler(ThresholdEvent.THRESHOLD_UNDERRUN, HANDLER);
	}

	public void removeOnThresholdUnderrun(final EventHandler<ThresholdEvent> HANDLER) {
		removeEventHandler(ThresholdEvent.THRESHOLD_UNDERRUN, HANDLER);
	}
	...
	public static class ButtonEvent extends Event {
		...
		// ******************** Constructors
		// **************************************
		public ButtonEvent(final EventType<ButtonEvent> TYPE) {
			super(TYPE);
		}
	...
	public static class ThresholdEvent extends Event {
		...
		// ******************** Constructors
		// **************************************
		public ThresholdEvent(final EventType<ThresholdEvent> TYPE) {
			super(TYPE);
		}
