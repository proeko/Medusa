...
public class DashboardSkin extends GaugeSkinBase {
	...
	private void initGraphics() {
		...
		barBackgroundOuterArc = new ArcTo(0.675 * PREFERRED_HEIGHT, 0.675 * PREFERRED_HEIGHT, 0, PREFERRED_WIDTH,
				0.675 * PREFERRED_HEIGHT, true, true);
		barBackgroundLineToInnerArc = new LineTo(0.72222 * PREFERRED_WIDTH, 0.675 * PREFERRED_HEIGHT);
		barBackgroundInnerArc = new ArcTo(0.3 * PREFERRED_HEIGHT, 0.3 * PREFERRED_HEIGHT, 0, 0.27778 * PREFERRED_WIDTH,
				0.675 * PREFERRED_HEIGHT, false, false);
		...
		pane = new Pane(unitText, titleText, valueText, minText, maxText, barBackground, dataBar, threshold,
				thresholdText);
		pane.setBorder(new Border(new BorderStroke(gauge.getBorderPaint(), BorderStrokeStyle.SOLID, CornerRadii.EMPTY,
				new BorderWidths(gauge.getBorderWidth()))));
		pane.setBackground(
				new Background(new BackgroundFill(gauge.getBackgroundPaint(), CornerRadii.EMPTY, Insets.EMPTY)));
	...
	protected void handleEvents(final String EVENT_TYPE) {
		...
		if ("RECALC".equals(EVENT_TYPE)) {
			...
			if (gauge.isInteractive()) {
				addMouseEvents(dataBar);
				addMouseEvents(barBackground);
				addMouseEvents(threshold);
			}
	...
	private void setBar(final double VALUE) {
		...
		valueText.relocate((width - valueText.getLayoutBounds().getWidth()) * 0.5,
				0.615 * height + (0.3 * height - valueText.getLayoutBounds().getHeight()) * 0.5);
	...
	protected void resize() {
		...
		if (width > 0 && height > 0) {
			...
			if (titleText.getLayoutBounds().getWidth() > maxWidth)
				Helper.adjustTextSize(titleText, maxWidth, size * 0.12);
			...
			if (valueText.getLayoutBounds().getWidth() > maxWidth)
				Helper.adjustTextSize(valueText, maxWidth, size * 0.24);
			valueText.relocate((width - valueText.getLayoutBounds().getWidth()) * 0.5,
					0.615 * height + (0.3 * height - valueText.getLayoutBounds().getHeight()) * 0.5);
			...
			if (minText.getLayoutBounds().getWidth() > maxWidth)
				Helper.adjustTextSize(minText, maxWidth, size * 0.12);
			...
			if (maxText.getLayoutBounds().getWidth() > maxWidth)
				Helper.adjustTextSize(maxText, maxWidth, size * 0.12);
			maxText.relocate(((0.27778 * width) - maxText.getLayoutBounds().getWidth()) * 0.5 + 0.72222 * width,
					0.7 * height);
			...
			thresholdText.relocate(
					centerX - (thresholdText.getLayoutBounds().getWidth() * 0.5)
							+ thresholdTextRadius * Math.sin(-Math.toRadians(thresholdAngle)),
					centerX - (thresholdText.getLayoutBounds().getWidth() * 0.5)
							+ thresholdTextRadius * Math.cos(-Math.toRadians(thresholdAngle)));
		}
	}
	...
	protected void redraw() {
		pane.setBorder(new Border(new BorderStroke(gauge.getBorderPaint(), BorderStrokeStyle.SOLID, CornerRadii.EMPTY,
				new BorderWidths(gauge.getBorderWidth() / 250 * size))));
		pane.setBackground(
				new Background(new BackgroundFill(gauge.getBackgroundPaint(), CornerRadii.EMPTY, Insets.EMPTY)));
